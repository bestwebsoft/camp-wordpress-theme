...
function camp_add_post_to_slider_ap() {
	if ( isset( $_POST['camp_add_to_slider'] ) )
		add_post_meta( get_the_ID(), 'camp_in_slider', 'yes', true ); /* add post to slider */
	else
		delete_post_meta( get_the_ID(), 'camp_in_slider' ); /* remove post from slider */
}
...
function camp_comment_callback() { ?>
	<li <?php comment_class(); ?> id = "comment-<?php comment_ID(); ?>">
		<div class = "comment-body" id = "div-comment-<?php comment_ID(); ?>">
			...
			<?php comment_text(); ?>
			<div class = "reply">
			<?php comment_reply_link( array( 'add_below' => 'div-comment', 'depth' => 1, 'max_depth' => 10, 'reply_text' => __( 'Reply', 'camp' ) ) );
